...
export default function SavingGoals({ triggerRerender }) {
  ...
  useEffect(() => {
    fetchData("http://localhost:3001/savings/goal").then((data) => {
      setGoals(data.goals);
    });
  }, [triggerRerender]);
  ...
  const handleDelete = async (goal) => {
    ...
    if (confirm) {
      fetchData("http://localhost:3001/savings/goal", "DELETE", {
        id: goal.id,
        userId: goal.userId,
      })
        .then((successData) => {
  ...
  const handleDeposit = async (goal) => {
    ...
    let depositAmount = parseFloat(
      await pops.inputPop(`Enter deposit amount for ${goal.name}:`)
    );
