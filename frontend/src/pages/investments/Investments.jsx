...
export default function Investments() {
  ...
  const toggleInvestmentOpen = () => {
    setInvestmentOpen((prev) => !prev);
  };

  const handleOpenInvestment = (investment) => {
    setIndividualInvestment(investment);
    toggleInvestmentOpen();
  };
  ...
  const growthPercentage = useCallback((investment) => {
    return (
      ((investment.currentValue - investment.invested) / investment.invested) *
      100
    );
  }, []);
  return (
    <>
      {individualInvestment.id && (
        <IndividualInvestment
          toggleModal={toggleInvestmentOpen}
