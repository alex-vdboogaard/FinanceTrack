    ...
    return (
        <>
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "start", marginTop: "50px" }}>
                {expenses.length > 0 && <PieChart maxWidth={"400px"} title={"Expenses"} backgroundColors={expenseColours} labels={expenseLabels} data={expenseData} />}
                {income.length > 0 && <PieChart maxWidth={"400px"} title={"Income"} backgroundColors={incomeColours} labels={incomeLabels} data={incomeData} />}
                {income.length > 0 && expenses.length > 0 && <BarChart margin={"0px"} style={{ flexBasis: "100%" }} title="Income vs Expenses" group1Label="Income" group2Label="Expenses" items={barchartItems} />}
            </div>
