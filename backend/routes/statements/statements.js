...
router.get("/", (req, res) => {
  ...
  connection.query(sqlTotalCount, [req.session.userId], (err, countResults) => {
    if (err) return res.status(500).json({ message: err.message });
    ...
    const sqlStatements = `SELECT s.pdf_blob, s.id, f.id AS folder_id, s.name, s.folder_id, f.name as folder_name FROM Statement AS s LEFT JOIN Folder AS f ON s.folder_id = f.id WHERE s.user_id = ? LIMIT ? OFFSET ?`;
...
router.get("/search/:query", (req, res) => {
  ...
  const sql = `SELECT s.id, s.pdf_blob, f.id AS folder_id, s.name, s.folder_id, f.name as folder_name FROM Statement AS s LEFT JOIN Folder AS f ON s.folder_id = f.id WHERE s.user_id = ${req.session.userId} AND s.name LIKE '%${query}%'`;
  ...
  const fileUploadPromises = req.files.map((file) => {
    return new Promise((resolve, reject) => {
      fs.readFile(file.path, (err, data) => {
        ...
        connection.query(
          sql,
          [file.originalname, data, req.session.userId, parent_folder_id],
          (err, result) => {
            if (err) {
              fs.unlink(file.path, () => {});
