...
router.get("/", (req, res) => {
    ...
    connection.query(
        ...
        (err, countResults) => {
            ...
            const sqlStatements = `SELECT s.id, f.id AS folder_id, s.name, s.folder_id, f.name as folder_name FROM Statement AS s LEFT JOIN Folder AS f ON s.folder_id = f.id WHERE s.user_id = ? LIMIT ? OFFSET ?`;
