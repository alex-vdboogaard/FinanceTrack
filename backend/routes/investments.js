...
router.post("/", (req, res) => {
  ...
  connection.query(query, values, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error creating the investment." });
    }
...
router.put("/", (req, res) => {
  ...
  connection.query(query, (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ message: `Error updating the investment: ${err.message}` });
    }
...
router.delete("/", (req, res) => {
  ...
  if (req.userId !== user_id) {
    return res
      .status(403)
      .json({ message: "You may not delete this investment" });
  }
  ...
  connection.query(query, (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ message: `Error deleting the investment: ${err.message}` });
    }
