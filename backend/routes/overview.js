...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const assets = assetResults.map(
      (a) => new Asset(a.id, a.name, a.boughtFor, a.currentValue, a.type)
    );
    ...
    const investments = investmentsResults.map(
      (i) =>
        new Investment(i.id, i.description, i.invested, i.currentValue, i.type)
    );
    ...
    const bankAccounts = bankAccountsResults.map(
      (a) => new BankAccount(a.id, a.description, a.balance, a.type)
    );
