...
router.post("/", (req, res) => {
    ...
    connection.query(query, [req.session.userId, month, year], (err, results) => {
        if (err) {
            res.status(500).json({ message: "Error reading from the database" });
            return;
        }
        ...
        if (results.length > 0) {
            res.status(409).json({ message: "Review for the specified month and year already exists" });
            return;
        }

        const query = `INSERT INTO Monthly_Review (user_id, income, expenses, savings, invested, month, year) VALUES (?, ?, ?, ?, ?, ?, ?)`;

        connection.query(query, [req.session.userId, income, expenses, saved, invested, month, year], (err, results) => {
            if (err) {
